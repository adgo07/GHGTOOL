...
for _prefix, _clause in (("calcination", "B.3；8.1"), ("baking", "B.4；8.1"), ("graphitization", "B.5；8.1")):
    for _item in (
        ...
        _spec(
            ...
            FieldDataType.ENUM,
            required=True,
            help_text="非收到基数据必须提供换算证据并归一至收到基。",
            standard_clause=_clause,
            source_location=_RECEIVED_BASIS,
            advanced=True,
        ),
        ...
        _spec(
            ...
            "基准证明定位",
            FieldDataType.TEXT,
            help_text="填写证明编号或来源定位；非收到基输入时必填。",
            standard_clause=_clause,
            source_location=_RECEIVED_BASIS,
            advanced=True,
        ),
...
for _item in (
    ...
    _spec("electricity.acquisition", "电力取得方式", FieldDataType.ENUM, required=True, help_text="外购和自发自用独立于电力属性。", standard_clause="B.8；G05 电力规则", source_location=_STANDARD),
    _spec("electricity.attribute", "电力属性", FieldDataType.ENUM, required=True, help_text="常规、非化石能源和化石能源为每条明细的属性。", standard_clause="B.8；G05 电力规则", source_location=_STANDARD),
    _spec("electricity.proof_type", "非化石电力证明类型", FieldDataType.ENUM, help_text="非化石能源电力须按取得方式提供对应证明。", standard_clause="B.8；G05 电力规则", source_location=_STANDARD, advanced=True),
    _spec("electricity.proof_status", "非化石电力证明状态", FieldDataType.ENUM, help_text="证明无效或缺失时不得采用零因子。", standard_clause="B.8；G05 电力规则", source_location=_STANDARD, advanced=True),
):
    _register(_item)
...
for _item in (
    ...
    _spec("heat_factor", "热力排放因子", FieldDataType.READONLY_PARAMETER, unit="tCO₂/GJ", help_text="候选值来自 Canonical 参数库，来源、审核状态和选择理由只读展示。", standard_clause="附录 C.3；B.9", source_location=_STANDARD),
    _spec("heat_factor_selection_reason", "热力因子选择理由", FieldDataType.TEXT, help_text="选择非推荐候选值时填写人工确认理由。", standard_clause="G05 参数解析规则；B.9", source_location=_STANDARD, advanced=True),
